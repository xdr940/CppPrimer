...
int (*p2)[10] = &arr; // p2 points to an array of ten ints ，必须和指向数组的规格相同

typedef int arrT[10]; // arrT is a synonym（同义词） for the type array of ten ints

// two ways to declare function returning pointer to array of ten ints
arrT* func(int i);               // use a type alias（别名）
...
// returns a pointer to an array of five int elements 函数指针数组
int(*arrPtr(int i))[5]
{
...
int main()
{
	...
	int (*arrP)[5] = arrPtr(99);  // arrP points to an array of five ints
	int (&arrR)[5] = arrRef(40);  // arrR refers to an array of five ints
